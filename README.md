...
## Интерфейсы взаимодействующие с Model: 

  interface Product
  interface CartItem extends Product
  interface CartState
  interface OrderData
  type PaymentMethod = 'online' | 'upon-receipt';
...
## Методы:
 ...
 ## Поля:  
  ...
  protected element: HTMLElement - элемент модального окна
...
## Интерфейсы взаимодействующие с View:

  interface ComponentInterface
  interface ModalInterface extends ComponentInterface
  interface ComponentProps - обобщённый тип для данных, которые передаются в компоненты через props, и он позволяет расширять и передавать любые свойства без строгой типизации.
...
## Методы:
  ...
  ## Интерфейсы взаимодействующие с Presenter:

AppEventMap:

  'basket:open': void;            // открыть корзину
  'basket:add': Product;          // добавить товар в корзину
  'basket:remove': string;        // удалить товар из корзины (по id)
  'basket:clear': void;           // очистить корзину
  'order:submit': OrderData;      // отправить заказ
  'modal:open': HTMLElement;      // открыть модальное окно с переданным контентом
  'modal:close': void;            // закрыть модальное окно
